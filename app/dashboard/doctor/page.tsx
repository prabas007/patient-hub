...
import { MoodHalo } from "@/components/MoodHalo"
...
function DoctorPageInner() {
  ...
  const [moodActivated, setMoodActivated]   = useState(false)
  ...
  function handleReset() {
    ...
    setRawExperiences([]); setMoodActivated(false)
  }
  ...
  return (
    <div ref={wrapperRef} style={{ transition: "background-color 2.5s ease", padding: "20px", margin: "-20px" }}>
      <MoodHalo
        mood={moodActivated ? esi : "neutral"}
        active={moodActivated}
        cardRadius={20}
      >
            ...
            <AnimatePresence mode="wait">
                ...
                >
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                      {Object.entries(CONDITIONS).map(([name, { icon }]) => (
                        <button key={name}
                          ...
                          className="flex flex-col items-center gap-2 p-4 rounded-2xl border-2 text-sm font-medium transition-all duration-200 text-center leading-tight"
                          style={condition === name
                            ? { borderColor: "var(--theme-accent, #2563eb)", backgroundColor: "var(--theme-accent-soft, #eff6ff)", color: "var(--theme-accent, #2563eb)", transform: "scale(1.02)" }
                        ...
                        <div className="flex flex-wrap gap-3">
                          {stages.map((s) => (
                            <button key={s} onClick={() => setStage(stage === s ? "" : s)}
                              ...
                              style={stage === s
                                ? { borderColor: "var(--theme-accent, #2563eb)", backgroundColor: "var(--theme-accent-soft, #eff6ff)", color: "var(--theme-accent, #2563eb)" }
                                : { borderColor: "#e5e7eb", backgroundColor: "white", color: "#4b5563" }}
                            >
                              {s}
                            </button>
                        ...
                        >
                          {ESI_OPTIONS.map((opt) => (
                            <MoodHalo
                              key={opt.value}
                              mood={opt.value}
                              active={moodActivated && esi === opt.value}
                              cardRadius={16}
                            >
                              <button
                                onClick={() => { setEsi(opt.value); setEmotion(opt.value); setMoodActivated(true) }}
                                className={`w-full p-4 rounded-2xl border-2 text-left transition-all duration-150
                                  ${moodActivated && esi === opt.value
                    ...
                    {condition && !voiceDetected && (
                      ...
                      >
                        ...
                        <input
                          ...
                          className="w-full max-w-sm px-4 py-2.5 rounded-xl border-2 text-sm border-stone-200 bg-white text-stone-700 placeholder-stone-400 focus:outline-none focus:border-amber-400 transition-colors"
                        />
                      </motion.div>
                    )}
                  </AnimatePresence>
                  ...
                  <div className="relative my-8">
                    ...
                    <div className="relative flex justify-center">
                      <span className="bg-white px-4 text-xs text-stone-400 uppercase tracking-widest font-medium">or use your voice</span>
                    </div>
                  </div>
                  ...
                  <AnimatePresence mode="wait">
                    {isProcessing && (
                      ...
                      >
                        <MoodHalo mood={esi} active={true} cardRadius={9999}>
                          <div className="w-16 h-16 rounded-full flex items-center justify-center"
                            style={{ backgroundColor: "var(--theme-accent-soft, #eff6ff)" }}>
                            <svg className="animate-spin w-7 h-7" fill="none" viewBox="0 0 24 24" style={{ color: "var(--theme-accent, #2563eb)" }}>
                              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                            </svg>
                          </div>
                        </MoodHalo>
                        <p className="text-sm font-medium" style={{ color: "var(--theme-accent, #2563eb)" }}>Analyzing your voice…</p>
                        <p className="text-xs text-stone-400">Transcribing and detecting your condition</p>
                      </motion.div>
                    )}

                    {!isProcessing && voiceDetected && extractedInfo && (
                      <motion.div key="voice-result" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                        style={{ padding: "20px", margin: "-20px" }}
                      >
                        <MoodHalo mood={esi} active={true} cardRadius={16}>
                          <div className="rounded-2xl border p-5"
                            ...
                            <div className="flex items-start justify-between gap-4">
                              ...
                              <button
                                onClick={() => { setVoiceDetected(false); setTranscript(""); setExtractedInfo(null); setEmotion("neutral"); setMoodActivated(false) }}
                                className="text-xs text-stone-400 hover:text-stone-600 underline shrink-0"
                              >Re-record</button>
                            </div>

                            {(extractedInfo.condition || extractedInfo.stage) && (
                              <div className="mt-3 pt-3 border-t flex flex-wrap gap-2 items-center" style={{ borderColor: "var(--theme-border)" }}>
                                <span className="text-xs text-stone-500">Detected:</span>
                                {extractedInfo.condition && (
                                  <span className="text-xs px-2 py-0.5 rounded-full font-medium text-white" style={{ backgroundColor: "var(--theme-accent)" }}>
                                    {extractedInfo.condition}
                                  </span>
                                )}
                                {extractedInfo.stage && (
                                  <span className="text-xs px-2 py-0.5 rounded-full font-medium border" style={{ borderColor: "var(--theme-accent)", color: "var(--theme-accent)" }}>
                                    {extractedInfo.stage}
                                  </span>
                                )}
                            ...
                            <div className="mt-3 pt-3 border-t" style={{ borderColor: "var(--theme-border)" }}>
                              <p className="text-xs text-stone-500 mb-2">Your location (optional)</p>
                              <input type="text" value={region} onChange={(e) => setRegion(e.target.value)}
                                placeholder="e.g. San Francisco, Chicago…"
                                className="w-full max-w-xs px-3 py-2 rounded-lg border text-sm border-stone-200 bg-white text-stone-700 placeholder-stone-400 focus:outline-none focus:border-amber-400 transition-colors"
                              />
                            </div>
                          </div>
                        </MoodHalo>
                    ...
                    {!isProcessing && !voiceDetected && isRecording && (
                      ...
                      >
                        <div className="relative">
                          ...
                          <button onClick={stop}
                            className="relative w-16 h-16 rounded-full bg-red-500 flex items-center justify-center text-white shadow-xl hover:bg-red-600 active:scale-95 transition-all">
                    ...
                    {!isProcessing && !voiceDetected && !isRecording && (
                      ...
                      >
                        <button onClick={start}
                          className="w-16 h-16 rounded-full flex items-center justify-center text-white shadow-xl transition-all duration-200 hover:scale-105 active:scale-95"
                          style={{ backgroundColor: "var(--theme-btn, #2563eb)", boxShadow: "0 8px 30px color-mix(in srgb, var(--theme-accent, #2563eb) 30%, transparent)" }}
                        >
                  ...
                  <AnimatePresence>
                    {canSearch && !isProcessing && (
                      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mt-10">
                        <button onClick={() => runSearch()}
                          className="text-white font-semibold px-8 py-3.5 rounded-2xl transition-all duration-150 flex items-center gap-2 text-base active:scale-95"
                          style={{ backgroundColor: "var(--theme-btn, #2563eb)", boxShadow: "0 8px 25px color-mix(in srgb, var(--theme-accent, #2563eb) 25%, transparent)" }}
                        >
                          Find Matching Doctors <span>→</span>
                        </button>
                        {!stage && <p className="text-xs text-stone-400 mt-2">No stage selected — showing doctors across all stages</p>}
                      </motion.div>
                    )}
                  </AnimatePresence>
                </motion.div>
              ...
              {step === "results" && (
                <motion.div key="results"
                  ...
                  exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.3 }}
                >
                  <button onClick={handleReset} className="text-sm hover:underline mb-3 flex items-center gap-1"
                    style={{ color: "var(--theme-accent, #2563eb)" }}>← Change selection</button>

                  <h2 className="text-3xl font-bold mb-1" style={{ color: "var(--theme-text, #111827)" }}>Doctor Recommender</h2>
                  <p className="mb-6 text-sm" style={{ color: "color-mix(in srgb, var(--theme-text, #6b7280) 60%, transparent)" }}>
                    Results for <span className="font-semibold" style={{ color: "var(--theme-text)" }}>{condition}</span>
                    {stage && <><span style={{ color: "var(--theme-text)" }}>{" · "}</span><span className="font-semibold" style={{ color: "var(--theme-text)" }}>{stage}</span></>}
                    {!stage && <span className="ml-1 text-xs text-stone-400 italic">· all stages</span>}
                    {region && <span className="ml-1 text-xs text-stone-400">· near {region}</span>}
                    {voiceDetected && (
                  ...
                  {error && <div className="text-red-600 bg-red-50 border border-red-200 rounded-xl p-4 mb-6">Failed to load recommendations: {error}</div>}
                  ...
                  {!isLoading && ragSummary?.narrative && (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <button onClick={() => speak(ragSummary.narrative)} disabled={ttsLoading}
                          className="ml-auto flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-xl border transition-all duration-150 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                          style={{ borderColor: "var(--theme-accent)", color: ttsPlaying ? "white" : "var(--theme-accent)", backgroundColor: ttsPlaying ? "var(--theme-accent)" : "var(--theme-accent-soft, transparent)" }}>
                          {ttsLoading
                            ...
                            : <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3A4.5 4.5 0 0 0 14 7.97v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/></svg>
                          }
                          {ttsLoading ? "Loading…" : ttsPlaying ? "Stop" : "Read aloud"}
                        </button>
                      </div>
                      <p className="text-sm leading-relaxed" style={{ color: "color-mix(in srgb, var(--theme-text) 80%, transparent)" }}>
                        {ragSummary.narrative}
                      </p>
                      {ttsError && <p className="text-xs text-red-400 mt-2">⚠ {ttsError}</p>}
                    </motion.div>
                  )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {isLoading
                      ...
                      ? <div className="col-span-2 text-center py-16 text-stone-400">
                          ...
                          <button onClick={handleReset} className="mt-4 hover:underline text-sm" style={{ color: "var(--theme-accent)" }}>← Go back and adjust</button>
                        </div>
...
export default function DoctorPage() {
  return <EmotionProvider><DoctorPageInner /></EmotionProvider>
}