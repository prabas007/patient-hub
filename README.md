...
| Vector DB | Actian VectorAI (HNSW index, cosine similarity, 3072-dim) |
...
|---|---|---|
| `patient_experiences` | Core RAG memory — patient narratives + outcome scores | 3072 |
| `doctor_profiles` | Doctor semantic profiles for recommendation | 3072 |
| `circle_posts` | Community condition-specific posts | 3072 |
| `emotional_states` | Future: adaptive UI personalization | 3072 |
...
|---|---|---|
| `GET` | `/health` | Health check + VectorAI ping |
...
| `POST` | `/search_filtered` | Filtered search (by condition, stage, etc.) |
| `GET` | `/count/{collection}` | Vector count in a collection |
| `DELETE` | `/delete/{collection}/{id}` | Delete a vector |
...
`POST /transcribe-form` — accepts multipart form with an `audio` field (WebM from browser `MediaRecorder`).
...
`POST /consensus` — fires 3 Gemini calls in parallel, returns per-persona verdicts + a consensus vote.
...
We used ChatGPT to assist with the planning process of this project, primarily for advice on which frameworks and languages to use.

We used Claude for coding assistance throughout development — for debugging and suggestions on how to improve our code.

---

## Disclaimer

> **CareLink is not a medical provider.** It does not diagnose, treat, or replace professional medical advice. For emergencies, call 911.
